...
void *thread_Func(void * par){
    ...
    int id =*((int*)par);           // mi prendo l'id dal parametro di funzione
    int lSezGeneral=strlen(stringa1)/N_THREAD;      //calcolo la lunghezza generale senza ter conto dell'ultima stringa
    int resto=0;
    ...
    int j=((lSezGeneral-1)+resto);              //setto il contatore a dove devo iniziare ad invertire la stringa
        ...
        if(id==(N_THREAD-1)){
            ...
            resto=((strlen(stringa1))%N_THREAD);
            j=((((N_THREAD-1)-id)*lSezGeneral)+resto);
...
int main(int argc, char const *argv[]){
    ...
    printf("sono il padre vado a creare mio figlio\n");
